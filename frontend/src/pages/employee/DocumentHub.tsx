...
function fmtRelative(iso?: string): string {
  ...
  if (diff < 3600)  return `Today, ${d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}`;
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`${s.bg} ${s.text} text-[11px] font-semibold px-[8px] py-[3px]
                      rounded-full leading-[16px] whitespace-nowrap`}>
      {s.label}
    </span>
  );
}
...
function DocCard({ doc }: { doc: HubDocument }) {
  ...
  return (
    <div
      onClick={() => doc.id && navigate(`/documents/viewer?doc_id=${doc.id}`)}
      className="bg-white border border-[#f1f5f9] rounded-[16px]
                 shadow-[0px_1px_4px_rgba(0,0,0,0.04)]
                 flex flex-col gap-[12px] p-[20px] cursor-pointer
                 hover:border-[#6366f1]/30 hover:shadow-[0px_4px_16px_rgba(99,102,241,0.08)]
                 transition-all duration-200"
    >
      {/* File icon + type label */}
      <div className="flex items-start justify-between">
        <img src={getFileIcon(doc.file_type)} alt={doc.file_type}
             className="w-[44px] h-[52px] object-contain" />
        <span className="text-[#94a3b8] text-[11px] font-semibold tracking-[0.5px] uppercase">
      ...
      <div className="flex flex-col gap-[2px]">
        <p className="text-[#111827] text-[13px] font-semibold leading-[18px] line-clamp-2">
          {doc.name}
        </p>
...
function DocRow({ doc }: { doc: HubDocument }) {
  ...
  return (
    <div
      onClick={() => doc.id && navigate(`/documents/viewer?doc_id=${doc.id}`)}
      className="flex items-center gap-[16px] px-[20px] py-[14px]
                 border-b border-[#f8fafc] last:border-0
                 hover:bg-[#f8fafc] cursor-pointer transition-colors"
    >
      <img src={getFileIcon(doc.file_type)} alt={doc.file_type}
           className="w-[32px] h-[38px] object-contain shrink-0" />
      ...
      <StatusBadge status={doc.status} />
      <span className="text-[#94a3b8] text-[12px] shrink-0 hidden sm:block">
...
function ReqItem({ item, onUpload }: { item: RequirementItem; onUpload: (id: string) => void }) {
  ...
  return (
    <div className={`flex flex-col gap-[4px] p-[12px] rounded-[10px] border
                     ${isMissing
      ...
      <div className="flex items-center gap-[10px]">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[#111827] text-[13px] font-semibold leading-[18px] truncate">
            {item.task_name}
          </p>
          <p className={`text-[11px] leading-[14px] ${
            isMissing ? "text-[#ef4444]"
      ...
      {isMissing && (
        <button
          ...
          className="mt-[4px] w-full h-[28px] rounded-[7px] text-white text-[12px] font-medium
...
export default function DocumentHub() {
  ...
  // ── Filters ───────────────────────────────────────────────────────────────
  const filters = ["All Documents", "H-1B Visa", "Personal"];
  ...
  function handleUploadToTask(taskId: string) {
    navigate(`/documents/upload?task_id=${taskId}`);
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden"
      ...
      <header className="bg-white border-b border-[#f1f5f9] shrink-0
                         ...
                         px-[24px] sm:px-[32px] h-[64px] gap-[16px]">

        {/* Title */}
        <h1 className="text-[#0f172a] text-[20px] font-bold tracking-[-0.5px] shrink-0">
          Document Hub
        </h1>

        {/* Filter tabs */}
        <div className="flex items-center gap-[4px] overflow-x-auto hide-scrollbar">
          {filters.map(f => (
            <button
              key={f}
              onClick={() => setActiveFilter(f)}
              className={`px-[14px] py-[6px] rounded-full text-[13px] font-medium
        ...
        <div className="flex items-center gap-[12px] shrink-0">
          ...
          <div className="relative hidden md:block">
            ...
            <input
              ...
              className="pl-[34px] pr-[12px] py-[7px] text-[13px] bg-[#f8fafc]
                         border border-[#f1f5f9] rounded-[10px] focus:outline-none
          ...
          <div className="hidden lg:flex items-center gap-[8px]">
            <img src={imgStorage} alt="" className="w-[16px] h-[16px] text-[#64748b]" />
            <div className="flex flex-col gap-[2px]">
          ...
          <button className="bg-white border border-[#e2e8f0] rounded-[10px]
                             ...
                             hover:bg-[#f8fafc] transition relative">
          ...
          <div className="flex items-center gap-[8px]">
            ...
            <div className="hidden lg:flex flex-col">
              <span className="text-[#0f172a] text-[13px] font-semibold leading-[16px]">
                {user ? `${user.first_name} ${user.last_name}` : "—"}
              </span>
              <span className="text-[#94a3b8] text-[11px] leading-[14px]">Employee</span>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* ── MAIN SCROLL AREA ── */}
      <main className="flex-1 overflow-y-auto px-[16px] sm:px-[24px] lg:px-[32px]
                       py-[24px] sm:py-[28px]">
        <div className="flex gap-[24px] items-start max-w-[1400px] mx-auto">

          {/* ═══════════════════════════════════════════════════════════════
              LEFT — Upload + Documents list (grows to fill)
          ═══════════════════════════════════════════════════════════════ */}
          <div className="flex flex-col gap-[24px] flex-1 min-w-0">

            {/* ── Upload zone ── */}
            <div className="bg-white border border-[#f1f5f9] rounded-[16px]
                            shadow-[0px_1px_4px_rgba(0,0,0,0.04)] p-[24px]">
              <div className="flex items-center justify-between mb-[16px]">
                <h2 className="text-[#0f172a] text-[16px] font-bold tracking-[-0.3px]">
                  Upload New Document
                </h2>
                <button className="text-[#6366f1] text-[13px] font-medium hover:underline">
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-[14px] flex flex-col
                            items-center justify-center gap-[10px] py-[32px] cursor-pointer
                            transition-all duration-200 ${
                  dragging
                    ? "border-[#6366f1] bg-[#f0f0ff]"
                    : uploading
                    ? "border-[#6366f1]/40 bg-[#fafafe]"
                    : "border-[#e2e8f0] bg-[#fafafe] hover:border-[#6366f1]/60 hover:bg-[#f5f5ff]"
                }`}
              >
                {uploading ? (
                  <>
                  ...
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[#94a3b8] text-[13px] mt-[2px]">
                        or click to browse your computer
                      </p>
                    </div>
                    <button
                      onClick={e => { e.stopPropagation(); fileRef.current?.click(); }}
                      className="bg-white border border-[#e2e8f0] text-[#374151] text-[13px]
                                 font-medium px-[20px] py-[8px] rounded-[8px]
              ...
              {uploadError && (
                <p className="text-[#ef4444] text-[13px] mt-[10px]">{uploadError}</p>
              )}
            ...
            {/* ── Recent Documents ── */}
            <div className="bg-white border border-[#f1f5f9] rounded-[16px]
                            shadow-[0px_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
              ...
              <div className="flex items-center justify-between px-[20px] py-[16px]
                ...
                <div className="flex items-center gap-[2px]">
                  <button
                    ...
                    className={`p-[7px] rounded-[7px] transition ${
                      viewMode === "list"
                        ? "bg-[#6366f1] text-white"
                        : "text-[#94a3b8] hover:bg-[#f1f5f9]"
                    }`}
                  >
                    <img src={imgListView} alt="list"
                         className={`w-[16px] h-[16px] ${viewMode === "list" ? "brightness-0 invert" : ""}`} />
                  </button>
                  <button
                  ...
                  >
                    <img src={imgGridView} alt="grid"
                         className={`w-[16px] h-[16px] ${viewMode === "grid" ? "brightness-0 invert" : ""}`} />
                  </button>
                </div>
              </div>
              ...
              {!isLoading && !error && documents.length === 0 && (
                <div className="flex flex-col items-center gap-[12px] py-[48px]">
                  ...
                  <p className="text-[#64748b] text-[14px]">No documents yet</p>
                </div>
              )}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              RIGHT sidebar — Requirements + Activity  (fixed 280px)
          ═══════════════════════════════════════════════════════════════ */}
          <div className="hidden lg:flex flex-col gap-[20px] w-[280px] shrink-0">
            ...
            {requirements && (
              <div className="bg-white border border-[#f1f5f9] rounded-[16px]
                              shadow-[0px_1px_4px_rgba(0,0,0,0.04)] p-[20px]">
                <div className="flex items-center justify-between mb-[16px]">
                  <h3 className="text-[#0f172a] text-[15px] font-bold tracking-[-0.3px]">
                    {requirements.visa_code} Requirements
                  </h3>
                  <span className="bg-[#f0fdf4] border border-[#d1fae5] text-[#059669]
                                   text-[11px] font-semibold px-[8px] py-[3px] rounded-full">
            ...
            <div className="bg-white border border-[#f1f5f9] rounded-[16px]
                            shadow-[0px_1px_4px_rgba(0,0,0,0.04)] p-[20px]">
              <h3 className="text-[#0f172a] text-[15px] font-bold tracking-[-0.3px] mb-[16px]">
                Recent Activity
              </h3>
                ...
                <div className="flex flex-col gap-[14px]">
                  {activity.map(item => (
                    <div key={item.id} className="flex gap-[10px] items-start">
                      <img src={imgActivityDot} alt=""
                           className="w-[8px] h-[8px] mt-[5px] shrink-0" />
                      <div className="flex flex-col gap-[2px] min-w-0">
                        <p className="text-[#0f172a] text-[12px] font-medium leading-[16px]">
                          {item.text}
                        </p>
                        <p className="text-[#94a3b8] text-[11px] leading-[14px]">
                          by {item.by} • {fmtRelative(item.timestamp)}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <button
                onClick={() => {}}
                className="mt-[16px] w-full text-center text-[#6366f1] text-[12px]
