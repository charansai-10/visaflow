...
function inviteDisplayValue(inv: InvitationResponse) {
  ...
  return inv.invite_token ? `https://app.visaflow.io/invite/${inv.invite_token}` : '—';
}
...
export default function HRInviteEmployees() {
  ...
  const linkUrl = linkInv?.invite_token ? `https://app.visaflow.io/invite/${linkInv.invite_token}` : null;
  ...
  const shareLink = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ url: linkUrl, title: 'Join us on VisaFlow' });
      } catch {
        //
      }
    } else {
  ...
  const onCopyRow = async (i: InvitationResponse) => {
    try {
      if (i.invite_code) {
        ...
        await navigator.clipboard?.writeText(`https://app.visaflow.io/invite/${i.invite_token}`);
        pushToast('success', 'Link copied to clipboard!', 'Invite link has been copied.');
      }
    } catch {
