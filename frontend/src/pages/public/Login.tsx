 ...
 * Blocks open-redirect attacks (e.g. //evil.com, https://evil.com)
 * by requiring the target to start with a single "/".
 */
...
export default function Login() {
  ...
  // ── Preserve the `?redirect=...` query param across login ─────────────────
  const [params] = useSearchParams();
  const redirectTo = params.get("redirect");
  const goAfterLogin = () => navigate(safeRedirectPath(redirectTo, "/dashboard"));
  ...
  function loginWithLinkedIn() {
    ...
    const state       = crypto.randomUUID();
    sessionStorage.setItem("linkedin_state", state);
    // Honor `?redirect=...` for the OAuth round-trip too
    sessionStorage.setItem("linkedin_post_login", safeRedirectPath(redirectTo, "/dashboard"));
    window.location.href =
      `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scope}&state=${state}`;
  }

  // Forward the redirect param to /signup so users who click "Sign up free"
  // and complete onboarding still land back on the invite page.
      ...
      <div className="flex-1 flex items-start lg:items-center justify-center px-4 sm:px-8 lg:px-12 xl:px-[136px] py-8 sm:py-12 bg-[#f9fafb]">
        <div className="flex flex-col gap-4 sm:gap-6 w-full max-w-[448px]">
          ...
          <div className="flex lg:hidden items-center justify-between mb-2">
            ...
            <Link to={signupHref} className="text-[#2563eb] font-semibold text-sm tracking-[-0.5px] hover:underline" style={{ fontFamily: "Inter, sans-serif" }}>
              Sign up free
            </Link>
          </div>
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <Link to={signupHref} className="text-[#2563eb] font-semibold text-sm sm:text-base tracking-[-0.5px] leading-6 text-center hover:underline" style={{ fontFamily: "Inter, sans-serif" }}>
                Sign up for free
              </Link>
            </div>
