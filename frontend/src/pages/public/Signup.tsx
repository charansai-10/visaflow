...
/**
 * sessionStorage key used to remember where to send the user after the full
 * signup flow (signup → verify-email → profile-setup → ???).
 * Read by onboarding pages on completion.
 */
export const POST_ONBOARDING_REDIRECT_KEY = "post_onboarding_redirect";
...
export default function Signup() {
  ...
  // ── Preserve ?redirect= across the full onboarding journey ────────────────
  const [params] = useSearchParams();
  const redirectTo = params.get("redirect");

  // Stash redirect target into sessionStorage so verify-email and profile-setup
  // pages can pick it up when they finish.
  useEffect(() => {
    const safe = safeRedirectPath(redirectTo, "");
    if (safe) sessionStorage.setItem(POST_ONBOARDING_REDIRECT_KEY, safe);
    // Note: we do NOT clear on unmount — onboarding pages need it later.
  }, [redirectTo]);
  ...
  // Forward redirect to /login when the user wants to sign in instead
    ...
    <div className="min-h-screen bg-[#f9fafb]">
      ...
      <header className="bg-white border-b border-[#e5e7eb] h-[60px] sm:h-[72px] flex items-center sticky top-0 z-20">
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-8 lg:px-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-xs sm:text-sm tracking-[-0.5px]">
            <span className="text-[#6b7280] hidden sm:inline">Already have an account?</span>
            <Link to={loginHref} className="text-[#2563eb] font-semibold hover:underline ml-1">Sign In</Link>
          </div>
        </div>
      </header>
